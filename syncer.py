...
def sync():
    ...
    for alias in ldap_alias_results:
        try:
            ...
            if not alias[0]:
                continue
            user_alias = ldap_alias_results[1]['mailAlternativeAddress'][0].result()
            ldap_active = True
            
          #  if api_user_alias != file_alias:
                logging.info(f"Checked user {alias}, add {user_alias}") 
            
            #(file_alias) = filedb.check_user(user_alias)
            #(api_user_alias) = api.check_user(user_alias)
            
        except Exception:
            logging.info(f"Fehler bei der Verarbeitung von {user_alias}")
            pass
